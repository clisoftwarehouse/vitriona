...
import { Tag, Copy, Plus, Trash2, Loader2, ToggleLeft, ToggleRight, TicketPercent } from 'lucide-react';
...
export function CouponsDashboard({ businessId }: { businessId: string }) {
  ...
  const fetchCoupons = async () => {
    setLoading(true);
    const result = await getCouponsAction(businessId);
    if (result.data) setCoupons(result.data as Coupon[]);
    setLoading(false);
  };
