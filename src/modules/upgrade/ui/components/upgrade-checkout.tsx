...
import { Check, Loader2, FileText, Sparkles, ArrowLeft, Building2, ArrowRight, CreditCard } from 'lucide-react';
...
export function UpgradeCheckout({ businesses, userEmail, preselectedBusinessId }: UpgradeCheckoutProps) {
  ...
  const [selectedBusiness, setSelectedBusiness] = useState(() => {
    if (preselectedBusinessId && businesses.some((b) => b.id === preselectedBusinessId)) {
  ...
  // Derive available plans for the selected business
  const currentBusiness = businesses.find((b) => b.id === selectedBusiness);
  const availablePlans = useMemo(() => {
    if (!currentBusiness) return PLANS;
    if (currentBusiness.plan === 'pro') return PLANS.filter((p) => p.key === 'business');
    return PLANS; // free → show both
  }, [currentBusiness]);

  const [selectedPlan, setSelectedPlan] = useState<PlanKey>(() => {
    const biz = preselectedBusinessId
      ? (businesses.find((b) => b.id === preselectedBusinessId) ?? businesses[0])
      : businesses[0];
    return biz?.plan === 'pro' ? 'business' : 'pro';
  });

  // When business changes, auto-select the first available plan
  const handleBusinessChange = (id: string) => {
    setSelectedBusiness(id);
    const biz = businesses.find((b) => b.id === id);
    if (biz?.plan === 'pro') {
      setSelectedPlan('business');
    } else if (selectedPlan === 'business' || selectedPlan === 'pro') {
  ...
  const canProceedPlan = selectedBusiness && selectedPlan;
      ...
      <Card className='mx-auto max-w-lg py-16'>
        <CardContent className='flex flex-col items-center text-center'>
          ...
          <p className='text-muted-foreground mt-2 max-w-sm text-sm'>
            Hemos recibido tu solicitud de upgrade al plan <strong>{plan.name}</strong>. Verificaremos tu pago y
            activaremos tu plan lo antes posible. Te notificaremos por correo electrónico.
          </p>
    ...
    return (
      <Card className='mx-auto max-w-lg py-16'>
        <CardContent className='flex flex-col items-center text-center'>
          ...
          <h2 className='mt-4 text-lg font-semibold'>No tienes negocios elegibles</h2>
          <p className='text-muted-foreground mt-1 max-w-sm text-sm'>
            Todos tus negocios ya tienen el plan más alto disponible, o no tienes negocios creados.
          </p>
    ...
    <div className='space-y-6'>
        ...
        <div className='space-y-6'>
          ...
          {businesses.length > 1 && (
            <div className='space-y-2'>
              <Label>Negocio a actualizar</Label>
                  ...
                  {businesses.map((b) => (
                    <SelectItem key={b.id} value={b.id}>
                      <span className='flex items-center gap-2'>
                        ...
                        {b.name}
                        <span className='text-muted-foreground text-[10px] uppercase'>({b.plan})</span>
                      </span>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className='bg-muted/50 flex items-center gap-3 rounded-lg border px-4 py-3'>
              ...
              <div>
                <p className='text-sm font-medium'>
                  {currentBusiness.name} — Plan actual:{' '}
          ...
          {/* Plan cards */}
          <div
            className={`grid gap-4 ${availablePlans.length === 1 ? 'mx-auto max-w-md sm:grid-cols-1' : 'sm:grid-cols-2'}`}
          >
      ...
      {step === 'payment' && (
        <div className='space-y-6'>
          ...
          <div>
            <Label className='mb-3 block text-base font-semibold'>Selecciona el método de pago</Label>
      ...
      {step === 'invoice' && (
        <div className='space-y-6'>
          ...
          <div className='space-y-4'>
            <div>
              <Label className='mb-3 block text-base font-semibold'>Comprobante de pago</Label>
            ...
            <div>
              <Label className='mb-3 block text-base font-semibold'>Datos de facturación</Label>
              <div className='grid gap-3 sm:grid-cols-2'>
                <div>
