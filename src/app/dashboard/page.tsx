...
import { Package, TrendingUp, DollarSign, TrendingDown, ArrowUpRight, ShoppingCart, MessageSquare } from 'lucide-react';
...
const DashboardPage = async () => {
  ...
  const revenueUp = stats ? stats.revenue.current >= stats.revenue.previous : true;
  const ordersUp = stats ? stats.orders.current >= stats.orders.previous : true;

  const statCards = [
    {
      label: 'Ingresos del mes',
      value: formatCurrency(stats?.revenue.current ?? 0, currency),
                ...
                <TableBody>
                  {recentOrders.map((order) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell className='font-medium'>{formatCurrency(parseFloat(order.total), currency)}</TableCell>
